...
backend k3s-db
  ...
  server k3s-db-01 k3s-db-01.lab.lan:3306 check on-marked-up shutdown-backup-sessions 
  server k3s-db-02 k3s-db-02.lab.lan:3306 check backup
```
...
- server k3s-db-01 k3s-db-01.lab.lan:3306 check on-marked-up shutdown-backup-sessions - последние два пункта обозначают, что когда данный основной сервер поднимается, балансировщик обрывает соединения со всеми серверами, отмеченными как backup. Это очень важно, т.к. сервера MySQL у нас работают в режиме master-master, и именно эта настройка не даст случиться сплит-брейну.
