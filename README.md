...
А вообще я уже разобрал этот кластер и поднял новый, с etcd вместо внешней БД и плавающим адресом прямо на серверах. Пока писал этот текст, хорошо освежил в памяти работу с HAProxy, keepalived и MySQL, но сама по себе схема все-таки такая себе. Такие дела.